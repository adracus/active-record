...
abstract class RelationManager {
  ...
  RelationManager(this.caller, this.relation, this.sourceCollection, this.targetCollection);
}

class HasManyManager extends RelationManager {
  HasManyManager(Model caller, Relation relation, Collection sourceCollection,
      Collection targetCollection)
      : super(caller, relation, sourceCollection, targetCollection);
  
  Future get() =>
      targetCollection.findByVariable({relation.variableOnTarget.name: caller.id});
...
class BelongsToManager extends RelationManager {
  BelongsToManager(Model caller, Relation relation, Collection sourceCollection,
      Collection targetCollection)
      : super(caller, relation, sourceCollection, targetCollection);
